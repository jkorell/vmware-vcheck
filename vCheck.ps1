...
$userMods = Get-ChildItem ($modHome + "*.ps1") | Sort-Object -Property Name
...
# Function names must match the file names
$modulesToCall = @()

foreach ($module in $userMods) {
  $modulesToCall += $module.Name.substring(0, $module.Name.LastIndexOf("."))
}
...
# Create report
$vCheckReport = Get-CustomHTML "$VIServer vCheck"
$vCheckReport += Get-CustomHeader0 ($VIServer.Name)

# Loop over all module names that are in the config folder
foreach ($module in $modulesToCall) {
  $vCheckReport += Invoke-Expression $module
}

$vCheckReport += Get-CustomHeader0Close
$vCheckReport += Get-CustomHTMLClose
